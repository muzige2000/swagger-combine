...
function mergeAllOf(allOf) {
  ...
  _.each(allOf, (item) => {
    ...
    let data = item["allOf"] ? mergeAllOf(item["allOf"]) : item
...
function fixForGoSwagger(obj) {
  let newObj = _.clone(obj)
  if (Array.isArray(obj)) {
    return _.map(newObj, (v) => {
      return fixForGoSwagger(v)
    })
  }
  if (typeof (newObj) === "object") {
    if (newObj["writeOnly"]) {
      delete newObj["writeOnly"]
    }
    if (newObj["example"]) {
      delete newObj["example"]
    }
    let a = _.mapValues(newObj, (v, k) => {
      if (Array.isArray(v)) {
        v = fixForGoSwagger(v)
      } else if (typeof (v) === "object") {
        if (v["allOf"]) {
          v = mergeAllOf(v["allOf"])
        }
        v = fixForGoSwagger(v)
      }
      return v
